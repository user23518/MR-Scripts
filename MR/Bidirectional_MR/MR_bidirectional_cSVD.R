...
# ↑ FIX 4 : WMH_FILE et T2D_FILE supprimés (non utilisés dans ce pipeline)
...
exposures <- list(
  ...
  list(name = "Parkinson's disease (Leonard 2025)",
       ...
       binary = TRUE, prev = 0.001, n_manual = NULL, primary_role = "outcome"),
  ...
  list(name = "Cardioembolic stroke (Mishra 2022)",
       ...
       binary = TRUE, prev = 0.002, n_manual = NULL, primary_role = "outcome"),
  ...
  list(name = "Small vessel stroke (Mishra 2022)",
       ...
       binary = TRUE, prev = 0.0025, n_manual = NULL, primary_role = "outcome"),
  ...
  list(name = "Severe internal carotid stenosis (Gummesson 2025)",
  ...
  list(name = "Type 2 diabetes (Mahajan 2018)",
       ...
       binary = TRUE, prev = 0.10, n_manual = NULL, primary_role = "exposure"),
...
# ── FIX 1 : build_col_map ajouté ici (était absent) ──────────────────────────
  ...
  dat <- tryCatch(
    {
      ...
      if (!"N" %in% names(d)) {
        if ("N_CASE" %in% names(d) && "N_CONTROL" %in% names(d)) {
          d$N <- d$N_CASE + d$N_CONTROL
          ts(sprintf("    N reconstruit : %d", d$N[1]))
        } else if (!is.null(exp$n_manual)) {
          d$N <- exp$n_manual
          ts(sprintf("    N manuel : %d", exp$n_manual))
        }
      }
      d
    },
...
for (exp_item in exp_loaded) {
  ...
  for (csvd_item in csvd_loaded) {
    ...
    res_A <- tryCatch(
      ...
      error = function(e) { ts(sprintf("  ✗ Erreur direction A : %s", conditionMessage(e))); NULL }
    )
    ...
    res_B <- tryCatch(
      ...
      error = function(e) { ts(sprintf("  ✗ Erreur direction B : %s", conditionMessage(e))); NULL }
    )
