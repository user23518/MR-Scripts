...
list(name = "Migraine (Hautakangas 2022)",
     ...
     binary = TRUE, prev = 0.14,
     n_cases_manual    = 38094,
     n_controls_manual = 210211,
 ...
 list(name = "Any stroke (Mishra 2022)",
  ...
  list(name = "Venous thromboembolism (Thibord 2022)",
       ...
       binary = TRUE, prev = 0.16, n_manual = NULL, primary_role = "exposure"), #n dans le data set 
...
# ← FIX : initialiser le flag
iv_threshold_relaxed <- FALSE

if (nrow(ivs) < MIN_IVS) {
    ivs <- exp_gwas[!is.na(pv) & pv < 5e-6, ]
    iv_threshold_relaxed <- TRUE
    ts(sprintf("  Relaxed (p < 5e-6): %d SNPs", nrow(ivs)))
  }

  if (nrow(ivs) < MIN_IVS) {
    ts("  ✗ [1/9] FAIL Instrument selection : 0 SNPs available — skipping")
    return(NULL)
  }
  ...
  if (n_avail < MIN_IVS) {
    ts(sprintf("  ✗ [1b/9] FAIL rsID match : only %d SNPs in outcome — skipping", n_avail, MIN_IVS))
    return(NULL)
  ...
  if (nrow(ivs) < MIN_IVS) {
    ts(sprintf("  ✗ [1c/9] FAIL Palindrome removal : 0 SNPs remaining — skipping"))
    return(NULL)
  }
...
exp_fmt <- tryCatch({
    ef <- do.call(format_data, eargs)
    ef$exposure <- exp_name
    ts("  ✓ [2/9] Exposure formatted")
    ef
  }, error = function(e) {
    ts(sprintf("  ✗ [2/9] FAIL Exposure format : %s", conditionMessage(e)))
    NULL
  })
...
# ── 3. LD clumping (EUR 1000G, r² < 0.001, 10 000 kb) ────────────────────
  ...
  exp_c <- tryCatch(
    ...
    error = function(e) { ts("⚠ [3/9] API clumping failed (using unclumped) : %s", conditionMessage(e)); exp_fmt } #Si l'API est inaccessible → affiche l'erreur et retourne tous les SNPs non clumpés
  )
  
    if (nrow(exp_c) < MIN_IVS) {
    ts(sprintf("  ✗ [3/9] FAIL Clumping : 0 IVs remaining — skipping"))
    return(NULL)
  }
  ts(sprintf("  ✓ [3/9] Clumping done : %d independent IVs", nrow(exp_c)))
...
if (nrow(out_sub) < MIN_IVS) {
    ts(sprintf("  ✗ [4/9] FAIL Outcome extraction : 0 SNPs found in outcome — skipping"))
    return(NULL)
  }
  ts(sprintf("  ✓ [4/9] Outcome extracted : %d / %d SNPs matched",
             nrow(out_sub), nrow(exp_c)))
...
out_fmt <- tryCatch({
    of <- do.call(format_data, oargs)
    of$outcome <- out_name
    ts("  ✓ [5/9] Outcome formatted")
    of
  }, error = function(e) {
    ts(sprintf("  ✗ [5/9] FAIL Outcome format : %s", conditionMessage(e)))
    NULL
  })
  if (is.null(out_fmt)) return(NULL)
   #appel la fonction format_data et rentre les arguments qu'on a inscrit dans la list 
...
# ── 6. Harmonise (action=3:,exclude ambiguous palindromic MAF 0.42–0.58) ────────────
  ...
  dat    <- tryCatch(
    harmonise_data(exp_c, out_fmt, action = 3),
    error = function(e) {
      ts(sprintf("  ✗ [6/9] FAIL Harmonisation : %s", conditionMessage(e)))
      NULL
    }
  )
  ...
  if (nrow(dat_mr) < MIN_IVS) {
    ts(sprintf("  ✗ [6/9] FAIL Harmonisation : 0 SNPs kept — skipping"))
    return(NULL)
  }
  ts(sprintf("  ✓ [6/9] Harmonisation done : %d SNPs kept", nrow(dat_mr)))
...
dir_res <- tryCatch(
    ...
    error = function(e) {
      ts(sprintf("  ⚠ [7/9] Steiger failed : %s", conditionMessage(e)))
      NULL
    }
  )
...
# sinon, on prend la première valeur de cette colonne (dir_res)
  ...
  if (is.null(dir_res)) {
    ts("  ⚠ [7/9] Steiger skipped — results set to NA")
  } else {
    ts(sprintf("  ✓ [7/9] Steiger done : correct = %s | p = %s",
...
res <- tryCatch(                   # ← tryCatch( ajouté ici
  mr(dat_use, method_list = c(
    "mr_ivw",
    "mr_ivw_fe",
    "mr_egger_regression",
    "mr_weighted_median"
  ...
  error = function(e) {
    ts(sprintf("  ✗ [8/9] FAIL Primary MR : %s", conditionMessage(e)))
    NULL
  }
)
...
# Weighted Median — méthode de sensibilité : 50% des instruments corrects
 ...
 het <- tryCatch(
    ...
    error = function(e) {
      ts(sprintf("  ⚠ [8b/9] Heterogeneity failed : %s", conditionMessage(e)))
      NULL
    }
  )
  if (!is.null(het) && nrow(het) > 0) {
    ...
    ts("  ⚠ [8b/9] Heterogeneity : no result")
  }
...
pleio <- tryCatch(
    ...
    error = function(e) {
      ts(sprintf("  ⚠ [8c/9] Pleiotropy failed : %s", conditionMessage(e)))
      NULL
    }
  )
...
#res = résultat de la MR 
 ...
 #Extrait les résultats des 4 méthodes
  ...
  if (is.na(rad$b)) {
    ts(sprintf("  ⚠ [9/9] Radial MR : no result (< 4 SNPs or error)"))
  } else {
    ts(sprintf("  ✓ [9/9] Radial done : %d outlier(s) removed | IVW p = %s",
               rad$n_out, fmt_p(rad$pval)))
  }
...
tryCatch({
  ht  <- min(50, max(6, nrow(dat_use) * 0.35 + 2))
    ...
    ggsave(sprintf("%s_scatter.pdf", pfx), mr_scatter_plot(res, dat_use)[[1]], width = 8, height = 6, limitsize = FALSE)
    ggsave(sprintf("%s_forest.pdf",  pfx), mr_forest_plot(sng)[[1]],           width = 8, height = ht,  limitsize = FALSE)
    ggsave(sprintf("%s_loo.pdf",     pfx), mr_leaveoneout_plot(loo)[[1]],      width = 8, height = ht, limitsize = FALSE)
    ggsave(sprintf("%s_funnel.pdf",  pfx), mr_funnel_plot(sng)[[1]],           width = 6, height = 6, limitsize = FALSE)  
    ts("  ✓ Plots saved")
  }, error = function(e) ts(sprintf("  ✗ FAIL Plots : %s", conditionMessage(e))))
...
# ──  Définir N_TESTS ────────────────────────────────────────────────────────
N_TESTS <- max(nrow(tbl_primary), 1L)   # ← FIX : défini ici
...
tbl_primary$IVW_p_Bonferroni <- fmt_p(bonf_threshold)


ts(sprintf("  N_TESTS (Bonferroni) = %d", nrow(tbl_primary)))
ts(sprintf("  Seuil Bonferroni     = %.2e", 0.05 / max(nrow(tbl_primary), 1)))
