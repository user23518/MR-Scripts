...
# ── Paths ─────────────────────────────────────────────────────────────────────
OUTDIR <-"/network/iss/debette/users/marine.huang/MR/results_steiger_NS_or_FALSE"
...
exposures <- list(
  ...
  list(name = "Parkinson's disease (Leonard 2025)",
       ...
       binary = TRUE, prev = 0.001, primary_role = "outcome",
       n_cases_manual = 34933, n_controls_manual = 3100),
  ...
  list(name = "Cardioembolic stroke (Mishra 2022)",
       ...
       binary = TRUE, prev = 0.002, n_cases_manual = 10804,
       n_controls_manual = 865389, primary_role = "outcome"),
  ...
  list(name = "Large artery stroke (Mishra 2022)",
       ...
       binary = TRUE, prev = 0.002, n_cases_manual = 6399,
       n_controls_manual = 865389, primary_role = "outcome"),
  ...
  list(name = "Small vessel stroke (Mishra 2022)",
       ...
       binary = TRUE, prev = 0.0025, n_cases_manual = 6811,
       n_controls_manual = 865389, primary_role = "outcome"),
  ...
  list(name = "Ischaemic stroke (Mishra 2022)",
       ...
       binary = TRUE, prev = 0.01, n_cases_manual = 59890,
       n_controls_manual = 865389, primary_role = "outcome"),
  ...
  list(name = "Any stroke (Mishra 2022)",
       ...
       binary = TRUE, prev = 0.015, n_cases_manual = 70720,
       n_controls_manual = 865389, primary_role = "outcome"),
  ...
  list(name = "Type 2 diabetes (Mahajan 2018)",
       ...
       binary = TRUE, prev = 0.10, n_cases_manual = 72209,
       n_controls_manual = 400308, primary_role = "exposure"),
...
safe_het <- function(het, method_name, col) {
  ...
  het_sub <- het[grepl(method_name, het$method, ignore.case = TRUE), ]
  ...
  tryCatch({
    ...
    tbl_inc <- if (file.exists(xlsx_path)) {
      tryCatch(
        ...
        error = function(e) {
          ts(sprintf("  ⚠ XLSX incrémental illisible — réinitialisé (%s)", conditionMessage(e)))
          NULL
        }
      )
    } else NULL
    ...
    openxlsx::setColWidths(wb, "Results", cols = seq_len(ncol(tbl_inc)), widths = "auto")
    openxlsx::freezePane(wb, "Results", firstRow = TRUE)
    openxlsx::saveWorkbook(wb, xlsx_path, overwrite = TRUE)

    ts(sprintf("  ✓ XLSX incrémental : %d ligne(s) → %s", nrow(tbl_inc), basename(xlsx_path)))
...
make_row <- function(d, analysis_type = "primary") {
  ...
  data.frame(
    ...
    Steiger_SNPs_removed       = safe_numeric(d$stg_n_removed),  # ← NEW
    Steiger_SNPs_kept          = safe_numeric(d$stg_n_kept),     # ← NEW
...
# ── Steiger-skip helpers ──────────────────────────────────────────────────────
is_steiger_skip     <- function(r) !is.null(r) && isTRUE(r$STEIGER_CONFIRMED_SKIP)
out_tsv_skipped     <- file.path(OUTDIR, "MR_steiger_confirmed_skipped.tsv")
...
run_direction <- function(
  ...
  iv_threshold_relaxed <- FALSE

  if (nrow(ivs) < MIN_IVS) {
    ivs <- exp_gwas[!is.na(pv) & pv < 1e-6, ]
    iv_threshold_relaxed <- TRUE
  ...
  # ── 7. Steiger : test global + filtre SNP si direction fausse ou NS ───────
  ts("  Steiger directionality test ...")

  # ── 7a. Reconstruire samplesize si absent ─────────────────────────────────
  ...
  compute_r <- function(beta, eaf, pval, n, ncase, ncontrol,
    ...
    if (is_binary) {
      ...
      if (is.null(n) || all(is.na(n))) return(rep(NA_real_, n_snps))
      pval_c <- pmax(pmin(pval, 1 - 1e-15), 1e-300)
      n_c    <- pmax(n, 2L)
      r <- TwoSampleMR::get_r_from_pn(p = pval_c, n = n_c)
    }
  ...
  # ── 7c. Test de directionnalité global ────────────────────────────────────
  dir_res <- tryCatch(
  ...
  stg_pval_num <- suppressWarnings(as.numeric(stg_pval))

  # Conditions
  steiger_confirmed      <- isTRUE(stg_dir) && !is.na(stg_pval_num) && stg_pval_num < 0.05
  steiger_direction_false <- !isTRUE(stg_dir)                          # FALSE ou NA
  steiger_nonsignificant  <- is.na(stg_pval_num) || stg_pval_num >= 0.05

  ts(sprintf(
    "  Steiger global : correct = %s | p = %s | → %s",
    stg_dir, fmt_p(stg_pval),
    if (steiger_confirmed)
      "SKIP (direction confirmée)"
    else if (steiger_direction_false && steiger_nonsignificant)
      "filtre SNP (direction fausse + non significatif)"
    else if (steiger_direction_false)
      "filtre SNP (direction fausse)"
    else
      "filtre SNP (non significatif)"
  ))

  # ── 7d. Skip si direction confirmée (TRUE + p < 0.05) ────────────────────
  if (steiger_confirmed) {
    ts("  ✗ Direction exclue — sera apportée par le script primaire au merge")
    return(list(
      STEIGER_CONFIRMED_SKIP = TRUE,
      exposure          = exp_name,
      outcome           = out_name,
      analysis_type     = NA_character_,
      stg_dir           = stg_dir,
      stg_pval          = stg_pval,
      n_snps_harmonised = nrow(dat_mr)
    ))
  }

  # ── 7e. Filtre SNP-niveau (direction FALSE OU non significative) ──────────
  stg_n_removed <- NA_integer_
  stg_n_kept    <- NA_integer_
  dat_stg_all   <- NULL

  dat_stg_all <- tryCatch(
    steiger_filtering(dat_mr),
    error = function(e) {
      ts(sprintf("  ! steiger_filtering() failed: %s", conditionMessage(e)))
      NULL
    }
  )

  if (!is.null(dat_stg_all) && "steiger_dir" %in% names(dat_stg_all)) {

    stg_pass      <- !is.na(dat_stg_all$steiger_dir) & dat_stg_all$steiger_dir == TRUE
    stg_n_removed <- sum(!stg_pass)
    stg_n_kept    <- sum(stg_pass)

    ts(sprintf(
      "  Steiger SNP-filter : %d / %d SNPs conservés (steiger_dir = TRUE) | %d retirés",
      stg_n_kept, nrow(dat_mr), stg_n_removed
    ))

    if (stg_n_removed > 0)
      ts(sprintf("  SNPs retirés : %s",
                 paste(dat_stg_all$SNP[!stg_pass], collapse = ", ")))

    dat_use <- if (stg_n_kept >= MIN_IVS) {
      ts(sprintf("  ✓ MR sur le set filtré Steiger (%d SNPs)", stg_n_kept))
      dat_stg_all[stg_pass, ]
    } else {
      ts(sprintf(
        "  ⚠ %d SNP(s) après filtre Steiger (< MIN_IVS = %d) — set complet utilisé (%d SNPs)",
        stg_n_kept, MIN_IVS, nrow(dat_mr)
      ))
      dat_mr
    }

  } else {
    ts("  ⚠ steiger_filtering() sans résultat — set harmonisé complet utilisé")
    dat_use <- dat_mr
  }
  ...
  res <- mr(dat_use, method_list = c(
    "mr_ivw", "mr_ivw_fe", "mr_egger_regression", "mr_weighted_median"
  ))

  ts(sprintf("  ── Primary MR results: %s → %s ──", exp_name, out_name))
  print(res[, c("method", "b", "se", "pval", "nsnp")])
  ts(sprintf("  Méthodes MR disponibles : %s", paste(res$method, collapse = " | ")))

  # ── 9. Heterogeneity & pleiotropy ─────────────────────────────────────────
  het <- tryCatch(
    mr_heterogeneity(dat_use),
    error = function(e) { ts("  ⚠ Heterogeneity: ", conditionMessage(e)); NULL }
  )

  if (!is.null(het) && nrow(het) > 0) {
    ts(sprintf("  ── Heterogeneity: %s → %s ──", exp_name, out_name))
    print(het[, intersect(c("method", "Q", "Q_df", "Q_pval"), names(het))])
  } else {
    ts("  Heterogeneity: no result")
  }

  pleio <- tryCatch(
    mr_pleiotropy_test(dat_use),
    error = function(e) { ts("  ⚠ Pleiotropy: ", conditionMessage(e)); NULL }
  )

  if (!is.null(pleio) && nrow(pleio) > 0) {
    ts(sprintf("  ── Pleiotropy (MR-Egger intercept): %s → %s ──", exp_name, out_name))
    print(pleio[, intersect(c("egger_intercept", "se", "pval"), names(pleio))])
  } else {
    ts("  Pleiotropy: no result")
  }
  ...
  sng <- tryCatch(mr_singlesnp(dat_use),   error = function(e) NULL)
  loo <- tryCatch(mr_leaveoneout(dat_use), error = function(e) NULL)

  tryCatch({
    data.table::fwrite(dat_use, sprintf("%s_dat_mr.tsv",     pfx), sep = "\t")
    data.table::fwrite(res,     sprintf("%s_mr_results.tsv", pfx), sep = "\t")
    if (!is.null(dat_stg_all))
      data.table::fwrite(dat_stg_all, sprintf("%s_steiger_snps.tsv", pfx), sep = "\t")
    if (!is.null(dir_res)) data.table::fwrite(dir_res, sprintf("%s_steiger.tsv",    pfx), sep = "\t")
    if (!is.null(het))     data.table::fwrite(het,     sprintf("%s_het.tsv",        pfx), sep = "\t")
    if (!is.null(pleio))   data.table::fwrite(pleio,   sprintf("%s_pleio.tsv",      pfx), sep = "\t")
    if (!is.null(sng))     data.table::fwrite(sng,     sprintf("%s_singlesnp.tsv",  pfx), sep = "\t")
    saveRDS(
      list(dat_use = dat_use, res = res, sng = sng, loo = loo,
           het = het, pleio = pleio, dir_res = dir_res,
           dat_stg_all = dat_stg_all,
           exp_name = exp_name, out_name = out_name),
      sprintf("%s_MR_objects.rds", pfx)
    )
  ...
  # ── 12. Plots ─────────────────────────────────────────────────────────────
  tryCatch({
    ht <- min(50, max(6, nrow(dat_use) * 0.35 + 2))
    ggplot2::ggsave(sprintf("%s_scatter.pdf", pfx), mr_scatter_plot(res, dat_use)[[1]], width = 8, height = 6,  limitsize = FALSE)
    if (!is.null(sng)) {
      ggplot2::ggsave(sprintf("%s_forest.pdf", pfx), mr_forest_plot(sng)[[1]], width = 8, height = ht, limitsize = FALSE)
      ggplot2::ggsave(sprintf("%s_funnel.pdf", pfx), mr_funnel_plot(sng)[[1]], width = 6, height = 6,  limitsize = FALSE)
    }
    if (!is.null(loo))
      ggplot2::ggsave(sprintf("%s_loo.pdf", pfx), mr_leaveoneout_plot(loo)[[1]], width = 8, height = ht, limitsize = FALSE)
  ...
  list(
    ...
    iv_threshold_relaxed = iv_threshold_relaxed,
    ivw    = ivw, ivw_fe = ivw_fe,
    ...
    stg_dir       = stg_dir,
    stg_pval      = stg_pval,
    stg_n_removed = stg_n_removed,  # ← NEW
    stg_n_kept    = stg_n_kept,     # ← NEW
...
for (exp_item in exp_loaded) {
  ...
  for (csvd_item in csvd_loaded) {
    ...
    # ── Steiger skip log direction A ──────────────────────────────────────
    if (is_steiger_skip(res_A)) {
      ts("  → Direction A: Steiger confirmed — logged and excluded")
      skip_row_A <- data.frame(
        Direction         = "A", Analysis_type = type_A,
        Exposure          = exp_cfg$name, Outcome = csvd_cfg$name,
        N_SNPs_harmonised = res_A$n_snps_harmonised,
        Steiger_correct   = as.character(res_A$stg_dir),
        Steiger_p         = fmt_p(res_A$stg_pval),
        stringsAsFactors  = FALSE
      )
      data.table::fwrite(skip_row_A, out_tsv_skipped,
                         sep = "\t", append = skip_header_written,
                         col.names = !skip_header_written)
    ...
    # ── Steiger skip log direction B ──────────────────────────────────────
    if (is_steiger_skip(res_B)) {
      ts("  → Direction B: Steiger confirmed — logged and excluded")
      skip_row_B <- data.frame(
        Direction         = "B", Analysis_type = type_B,
        Exposure          = csvd_cfg$name, Outcome = exp_cfg$name,
        N_SNPs_harmonised = res_B$n_snps_harmonised,
        Steiger_correct   = as.character(res_B$stg_dir),
        Steiger_p         = fmt_p(res_B$stg_pval),
        stringsAsFactors  = FALSE
      )
      data.table::fwrite(skip_row_B, out_tsv_skipped,
                         sep = "\t", append = skip_header_written,
                         col.names = !skip_header_written)
      skip_header_written <<- TRUE
      res_B <- NULL
...
if (length(valid_results) == 0) {
  ts("  ⚠ No valid MR results — check file paths and error messages above")
  ts("═══ Script ended with 0 results ═══")
...
tbl_primary$IVW_p_Bonferroni <- fmt_p(p.adjust(tbl_primary$IVW_p_raw, method = "bonferroni"))
...
data.table::fwrite(tbl,             out_tsv,                                                sep = "\t")
data.table::fwrite(tbl_primary,     file.path(OUTDIR, "FINAL_MR_primary.tsv"),             sep = "\t")
data.table::fwrite(tbl_sensitivity, file.path(OUTDIR, "FINAL_MR_sensitivity_reverse.tsv"), sep = "\t")
